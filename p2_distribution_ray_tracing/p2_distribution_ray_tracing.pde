...
float rays_per_pixel = 1;
float[] delta = {0.5, 0.5}; //random ray position in one pixel
// Lens lens;
float x0 = 0;
float y0 = 0;
boolean is_lens = false;
float lens_size = 0;
float lens_dist = 1;
...
void interpreter(String filename) {
  ...
  for (int i=0; i<str.length; i++) {
    ...
    if (token[0].equals("rays_per_pixel")) {
      rays_per_pixel = float(token[1]);
    }
    else if (token[0].equals("lens")) {
      lens_size = float(token[1]);
      lens_dist = float(token[2]);
      is_lens = true;
    }
      ...
      add_disk_light (x, y, z, rad, dx, dy, dz, r, g, b);
    }
      ...
      add_moving_sphere(r, x1, y1, z1, x2, y2, z2); 
    }
